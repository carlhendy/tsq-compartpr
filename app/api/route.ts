...
function extractStructuredInsights(html: string, scopeHint?: { start: number; end: number }) {
  ...
  function afterHeader(seg: string, headerPattern: string) {
    const re = new RegExp(
      `<(?:div|span)[^>]*class=["']hnGZye["'][^>]*>\\s*(?:${headerPattern})\\s*<\\/(?:div|span)>[\\s\\S]{0,240}?<(?:(?:div)|(?:span))[^>]*class=["']KtbsVc-ij8cu-fmcmS[^"']*["'][^>]*>([\\s\\S]*?)<\\/(?:div|span)>`,
      "i"
    );
  ...
  const payBlock = segment.match(
    new RegExp(
      `<(?:div|span)[^>]*class=["']hnGZye["'][^>]*>\\s*(?:Payment\\s+options|Payment\\s+methods)\\s*<\\/(?:div|span)>[\\s\\S]{0,240}?<span[^>]*class=["']KtbsVc-ij8cu-fmcmS[^"']*["'][^>]*>([\\s\\S]*?)<\\/span>`,
      "i"
    )
  );
  if (payBlock) {
  ...
  function gradeFor(seg: string, headerPattern: string): string {
    const re = new RegExp(
      `<(?:div|span)[^>]*class=["']hnGZye["'][^>]*>\\s*(?:${headerPattern})\\s*<\\/(?:div|span)>[\\s\\S]{0,340}?<span[^>]*class=["']rMOWke-uDEFge\\s+hnGZye[^"']*["'][^>]*>\\s*(Exceptional|Great|Good|Fair|Poor)\\s*<\\/span>`,
      "i"
    );
    const m = seg.match(re);
    return m ? stripTags(m[1]) : "";
...
function extractSignalsFromHtml(html: string, domain: string) {
  ...
  const near = domIdxRaw >= 0 ? ((s: string) => ({ start: Math.max(0, domIdxRaw - 28000), end: Math.min(s.length, domIdxRaw + 28000) }))(html) : null;
  ...
  // Ratings/Reviews — add robust Next-style fallbacks
  const h1Win = domIdxVis >= 0 ? win(visAll, domIdxVis, 7000) : visAll;
  ...
  let store_rating =
    ...
    // Visible 4.7 inside <p class="TRyy9-TY4T7c"><span ...>4.7</span>
    pick(htmlNear, [/<p[^>]*class=["'][^"']*TRyy9-TY4T7c[^"']*["'][^>]*>[\s\S]{0,80}?<span[^>]*>\s*(\d+(?:\.\d+)?)\s*<\/span>/i], 1) ||
    pick(html,     [/<p[^>]*class=["'][^"']*TRyy9-TY4T7c[^"']*["'][^>]*>[\s\S]{0,80}?<span[^>]*>\s*(\d+(?:\.\d+)?)\s*<\/span>/i], 1) ||
    // Classic variants
    pick(h1Win, [
    ...
    pick(visAll, [new RegExp("(?:\\b" + esc(domain) + "\\b)[\\s\\S]{0,3500}?(\\d\\.\\d|\\d)\\s*(?:[★⭐]|/\\s*5)?\\s*store\\s*rating\\b", "i")], 1);
  ...
  let review_count =
    ...
    pick(visAll,   [new RegExp("(?:\\b" + esc(domain) + "\\b)[\\s\\S]{0,4000}?(\\d{1,3}(?:,\\d{3})*)\\s*(?:reviews|ratings)", "i")], 1);
