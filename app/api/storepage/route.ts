...
function sanitizeNoise(s: string) {
  // remove "+ 2 more" etc so it can't confuse rating/review parsing
...
function extractStructuredInsights(htmlNear: string) {
  // Shipping
  const shipBlock = htmlNear.match(
    /<div[^>]*class=["']hnGZye["'][^>]*>\\s*Shipping\\s*<\\/div>\\s*<div[^>]*class=["']KtbsVc-ij8cu-fmcmS["'][^>]*>([\\s\\S]*?)<\\/div>/i
  );
  const shippingRaw = shipBlock ? stripTags(shipBlock[1]) : "";

  // Returns
  const retBlock = htmlNear.match(
    /<div[^>]*class=["']hnGZye["'][^>]*>\\s*Returns?\\s*<\\/div>\\s*<div[^>]*class=["']KtbsVc-ij8cu-fmcmS["'][^>]*>([\\s\\S]*?)<\\/div>/i
  );
  const returnsRaw = retBlock ? stripTags(retBlock[1]) : "";

  // Competitive pricing (grade only)
  const pricingHeader = htmlNear.search(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Competitive\\s+pricing\\s*<\\/div>/i);

  // Payment options: prefer expanded list <span class="NBMhyb">, else visible jsname=u5tB8
  let paymentsRaw = "";
  const payBlock = htmlNear.match(
    /<div[^>]*class=["']hnGZye["'][^>]*>\\s*Payment options\\s*<\\/div>\\s*<span[^>]*class=["']KtbsVc-ij8cu-fmcmS["'][^>]*>([\\s\\S]*?)<\\/span>/i
  );
  if (payBlock) {
  ...
  // Website quality (grade only)
  const websiteHeader = htmlNear.search(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Website\\s+quality\\s*<\\/div>/i);
  ...
  const wallets = Array.from(
    new Set(
      Array.from(
        paymentsRaw.matchAll(/\\b(Apple Pay|Google Pay|Shop Pay|PayPal|Afterpay|Klarna)\\b/gi)
      ).map((m) => m[1])
    )
  );
  const e_wallets = wallets.join(", ");

  // ---- Section grade labels (Exceptional/Great/Good/Fair/Poor) ----
  function gradeAfter(header: RegExp): string {
    const m = htmlNear.match(new RegExp(header.source + "[\\s\\S]{0,300}?(?:<span[^>]*class=[\"']rMOWke-uDEFge\\s+hnGZye[^\"']*[\"'][^>]*>\\s*(Exceptional|Great|Good|Fair|Poor)\\s*<\\/span>)", "i"));
    return m && m[1] ? stripTags(m[1]) : "";
  }

  const shipping_grade = gradeAfter(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Shipping\\s*<\\/div>/i);
  const returns_grade  = gradeAfter(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Returns?\\s*<\\/div>/i);
  const pricing_grade  = gradeAfter(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Competitive\\s+pricing\\s*<\\/div>/i);
  const payments_grade = gradeAfter(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Payment options\\s*<\\/div>/i);
  const website_grade  = gradeAfter(/<div[^>]*class=["']hnGZye["'][^>]*>\\s*Website\\s+quality\\s*<\\/div>/i);
  ...
  return {
    ...
    e_wallets,
    section_grades: {
      shipping: shipping_grade,
      returns: returns_grade,
      pricing: pricing_grade,
      payments: payments_grade,
      website: website_grade,
    },
  };
}
...
function extractSignalsFromHtml(html: string, domain: string) {
  ...
  // Logo
  const logoMatch = htmlNear.match(/<img[^>]*class=["']Kl6mye-l4eHX["'][^>]*src=["']([^"']+)["'][^>]*>/i);
  ...
  // Fallbacks
  if (!store_rating) {
    store_rating = pick(visAll, [
      new RegExp("(?:\\b" + esc(domain) + "\\b)[\\s\\S]{0,1600}?(\\d\\.\\d|\\d)\\s*(?:[★⭐]|/\\s*5)?\\s*store\\s*rating\\b", "i"),
    ], 1);
  }
  if (!review_count) {
    review_count = pick(visAll, [
      new RegExp("(?:\\b" + esc(domain) + "\\b)[\\s\\S]{0,2000}?(\\d{1,3}(?:,\\d{3})*)\\s*(?:reviews|ratings)", "i"),
    ], 1);
  }
